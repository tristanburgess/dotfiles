...
repo at `home/dot_claude/skills/guitar-library/`; synced to Notion via
the `guitar-library` skill's **Sync system doc** workflow.
...
Book rows are fully maintained by the `guitar-library` skill.
...
difficulty. Relation → Materials DB. Populated by the `guitar-library`
...
| Book preprocessing (OCR, split) | `guitar-library` skill | Claude Code CLI |
| Book indexing → Materials/Exercises | `guitar-library` skill | Claude Code CLI |
| Sync this doc → Notion | `guitar-library` skill's Sync workflow | Claude Code CLI |
...
1. Edit `home/dot_claude/skills/guitar-library/project-instructions.md`
...
1. Edit `home/dot_claude/skills/guitar-library/SKILL.md` in the
   dotfiles repo.
2. `chezmoi apply` — deploys to `~/.claude/skills/guitar-library/
   SKILL.md`.
3. Restart Claude Code CLI (or start a new session). Changes active.

### Editing this architecture doc

1. Edit `home/dot_claude/skills/guitar-library/
   system-architecture-and-updating.md`.
2. `chezmoi apply`.
3. In Claude Code CLI, run the `guitar-library` skill's
...
- **Claude Project file caps:** ≤ 30 MB per file AND ≤ 100 pages per
  ...
  must be split before upload (the `guitar-library` skill handles
  this).
